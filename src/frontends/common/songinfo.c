			     ...
			     struct uade_song *us, struct uade_config *uc)
{
  ...
  */
  ...
  while (dstoffs < dstlen) {
	...
	break;
      case 'B':
	snprintf(tmp, sizeof tmp, "%d", us->cur_subsong);
	dat = tmp;
	break;
      case 'C':
	snprintf(tmp, sizeof tmp, "%d", us->max_subsong);
	dat = tmp;
	break;
      case 'F':
	dat = bname;
	...
	if (us->min_subsong == us->max_subsong) {
	  ...
	  snprintf(tmp, sizeof tmp, "(%d/%d)", us->cur_subsong, us->max_subsong);
	}
	dat = tmp;
	break;
      default:
	fprintf(stderr, "Unknown identifier %%%c in song_title format: %s\n", 
		c, format);
	title[dstoffs] = 0;
	return 1;
      }
